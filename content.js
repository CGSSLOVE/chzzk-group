    ...
    groups.forEach(group => {
        ...
        if (group.streamers.length > 0) {
            group.streamers.forEach(streamer => {
                ...
                streamerItem.querySelector('.streamer-delete-btn').addEventListener('click', (e) => {
                    e.stopPropagation();
                    removeStreamerFromGroup(parseInt(e.target.dataset.groupId, 10), e.target.dataset.streamerId);
                });
        ...
        groupItem.querySelector('.group-drag-handle').addEventListener('mousedown', (e) => {
            ...
            draggedGroup = {
                ...
                id: groupContainer.dataset.groupId,
                originalY: e.clientY,
                ghostElement: null
            };
            document.body.style.cursor = 'grabbing';
        ...
        groupItem.querySelector('span').addEventListener('click', async () => {
            const list = document.getElementById(`group-${group.id}`);
            const currentlyExpanded = list.classList.toggle('expanded');
            const groups = await getGroups();
            const groupToToggle = groups.find(g => g.id === group.id);
            if (groupToToggle) {
        ...
        groupItem.querySelector('.edit-group-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            editGroupName(parseInt(e.target.dataset.groupId, 10));
        });
...
async function initializeUI() {
    ...
    if (followingContainer && followingContainer.parentNode) {
        ...
        followingContainer.parentNode.style.display = 'flex';
        followingContainer.parentNode.style.flexDirection = 'column';
        ...
        const data = await chrome.storage.sync.get('groupPositionTop');
